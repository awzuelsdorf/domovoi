...
if [ -d ~/domovoi ]; then
        ...
        (crontab -l | grep -Fv "run_domain_alert.sh" | grep -Fv "start_interceptor.sh"; echo "*/5 * * * * cd ~/domovoi && sudo ./run_domain_alert.sh 2>&1 | tee ~/domovoi/run_domain_alert.sh.log") | crontab -
        (crontab -l | grep -Fv "start_interceptor.sh"; echo "*/1 * * * * cd ~/domovoi && sudo ./start_interceptor.sh") | crontab -
else
